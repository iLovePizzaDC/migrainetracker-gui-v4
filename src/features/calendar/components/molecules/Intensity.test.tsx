...
describe('<Intensity />', () => {
	...
	it('renders the heading and the select input', async () => {
		render(<Intensity intensity={INTENSITY_TYPES.MEDIUM} onChange={mockOnChange} />);
	...
	it('calls onChange when selection changes', async () => {
		...
		expect(mockOnChange).toHaveBeenCalledWith(INTENSITY_TYPES.HIGH);
	});

	it('is disabled if prop is true', async () => {
		render(<Intensity intensity={INTENSITY_TYPES.MEDIUM} onChange={mockOnChange} disabled />);
