...
describe('<Medicine />', () => {
	...
	it('adapts effectiveness max value on taken slider change', async () => {
		...
		const handleOnChange: React.Dispatch<React.SetStateAction<typeof mockMedicines>> = (value) => {
			testMedicines = typeof value === 'function' ? value(testMedicines) : value;

			rerender(<Medicine medicines={testMedicines} onChange={handleOnChange} />);
		};

		const { rerender } = render(<Medicine medicines={testMedicines} onChange={handleOnChange} />);
	...
	it('is disabled if prop is true', async () => {
		mockUseCalendar(5, 10);
		const { default: Medicine } = await import('@/features/calendar/components/molecules/Medicine');
		render(<Medicine medicines={mockMedicines} onChange={mockOnChange} disabled />);
