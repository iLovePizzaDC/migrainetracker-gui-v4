...
describe('<Symptoms />', () => {
	...
	it('renders the heading', () => {
		render(<Symptoms symptoms={[]} onChange={mockOnChange} />);
	...
	it('renders pre checked symptoms', () => {
		render(
			<Symptoms symptoms={[SYMPTOM_TYPES.NOISE, SYMPTOM_TYPES.LIGHT]} onChange={mockOnChange} />,
		);
	...
	it('adds symptom when clicking unchecked option', async () => {
		...
		expect(mockOnChange).toHaveBeenCalledWith([SYMPTOM_TYPES.NOISE]);
	});

	it('removes symptom when clicking checked option', async () => {
		render(<Symptoms symptoms={[SYMPTOM_TYPES.NOISE]} onChange={mockOnChange} />);
	...
	it('is disabled if prop is true', () => {
		render(<Symptoms symptoms={[]} onChange={mockOnChange} disabled />);
