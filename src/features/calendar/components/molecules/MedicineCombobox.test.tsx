	...
	default: vi.fn(({ selected, onChange, renderOptionActions, disabled }: any) => {
		...
		return (
			<div data-testid='combobox'>
				...
				<button data-testid='select-both-btn' onClick={() => onChange([firstOption, secondOption])}>
...
describe('<MedicineCombobox />', () => {
	...
	it('renders pre-selected medicines', () => {
		render(<MedicineCombobox medicines={mockMedicines} onChange={mockOnChange} />);
	...
	it('calls setMedicines with correct structure when selecting', async () => {
		render(<MedicineCombobox medicines={[]} onChange={mockOnChange} />);
	...
	it('preserves taken and effectiveness for medicines that remain selected', async () => {
		...
		render(<MedicineCombobox medicines={customMedicines} onChange={mockOnChange} />);

		await user.click(screen.getByTestId('change-selection-btn'));

		expect(mockOnChange).toHaveBeenCalledWith(customMedicines);
	});
	...
	it('disables combobox when disabled prop is true', () => {
		render(<MedicineCombobox medicines={[]} onChange={mockOnChange} disabled={true} />);
